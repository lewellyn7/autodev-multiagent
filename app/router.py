...
from app.database import get_best_account, update_account_stats
...
class SmartRouter:
    ...
    async def route(self, model: str, messages: list, stream: bool = False, **kwargs) -> dict | AsyncIterator[dict]:
        """
        智能路由：
        1. 获取最优账户
        2. 尝试请求，失败则模型回退
        ...
        try:
            ...
            for fallback_model in fallback_models:
                try:
                    return await self._try_complete(
                        source, fallback_model, messages, stream, api_key, account, **kwargs
                    )
            ...
            for fallback_source in fallback_sources:
                try:
                    ...
                    fallback_key = (
                        fallback_account.get("tokens", {}).get("api_key") if fallback_account else kwargs.get("api_key")
                    )
                    if fallback_key:
                        return await self._try_complete(
                            fallback_source, model, messages, stream, fallback_key, fallback_account, **kwargs
                        )
                except ProviderError:
                    continue
    ...
    async def _try_complete(
        self, source: str, model: str, messages: list, stream: bool, api_key: str, account: dict = None, **kwargs
